...
def _coalesce_cutoff(*candidates) -> Optional[int]:
    """Return the first positive cutoff rank, ignoring None and zero."""
...
def evaluate_chancing_logic(
    student_rank: int, cutoff_2025: Optional[int], round_no: int
) -> Literal["High", "Medium", "Low", "Unlikely"]:
    """
    ...
    # Avoid division by zero or missing cutoffs
...
def generate_optimized_100_list(profile: StudentProfile, cutoff_dataset: List[Dict]) -> Dict:
    ...
    for item in cutoff_dataset:
        r2_cutoff = _coalesce_cutoff(
            item.get("round_2_cutoff"),
            item.get("round_1_cutoff"),
            item.get("round_3_cutoff"),
        ...
        r1_cutoff = _coalesce_cutoff(item.get("round_1_cutoff"), r2_cutoff, item.get("round_3_cutoff"))
        r3_cutoff = _coalesce_cutoff(item.get("round_3_cutoff"), r2_cutoff, item.get("round_1_cutoff"))
